...
#define ADC_BAT 36
#define RX8025T_INTV2 32 // 时钟芯片唤醒引脚
#define RX8025T_INTV1 15
...
#define wqy12 u8g2_font_wqy12_t_gb2312;
#define wqy13 u8g2_font_wqy13_t_gb2312;
#define wqy14 u8g2_font_wqy14_t_gb2312;
#define wqy15 u8g2_font_wqy15_t_gb2312;
#define wqy16 u8g2_font_wqy16_t_gb2312;
...
struct TXTRead
{
  String new_i;                // 新版本书解析文件
  String old_i;                // 旧版本书解析文件
  String Name;                 // 打开的书名称
  int zijianju = 8;            // 字间距
  int hangjianju = 8;          // 行间距
  int zitidaxiao = 15;         // 字体大小
  int yebianju = 15;           // 页边距
  int ScreenLength = 240;      // 屏幕物理尺寸宽
  int ScreenHeight = 416;      // 屏幕物理尺寸高
  uint32_t Get_p_file_int;     // 当前位置指针
  uint32_t Get_p_file_int_all; // 当前小说末尾指针位置
  int shiyongziti = 1;         // 使用字体的代号
  int beginpos;                // 底部小说名称开始位置
  // String shiyongziti = u8g2_font_wqy16_t_gb2312;
} TXTRead;
...
struct ReadModeDisplayMenu // 阅读模式菜单
{
  std::string Readmenu[8] = {
      ...
      "字体大小",
      "字体选择"};
  String text_show = "  本设备是以乐鑫公司生产的产品作为主控，集成了温湿度传感器，充放电电路，时钟等功能";
} RMDM;
...
uint64_t ChipIDxxxxx;  //ESP3的芯片ID
...
RTC_DATA_ATTR bool isConnectToOtherDevice = false; // 是否互联
RTC_DATA_ATTR bool isUnlockPro = false;            // 是否解锁增强
...
unsigned long HOU_fanyeqianweizhi;           // 用于记录翻页前的位置
unsigned long QIAN_fanyeqianweizhi;          // 用于记录翻页前的位置
uint8_t QIAN_fanyecishu = 0;                 // 用于记录向前翻页次数，大于一次的时候会进行数据交换
...
// 临时区域
//   温湿度显示区域
RTC_DATA_ATTR String T;
RTC_DATA_ATTR String Humidity;  // 修改H为Humidity以避免冲突
...
void SystemSleep();
// 我的解码TXT文件
void MyTXTDecode(bool place = true);
// 用于阅读设置界面动态展示调整效果
void displayTXT();
...
void WriteSDCardMessage(String FilePlace, String Filename, String FileContent, bool lazha = true);
...
void CLICK_CONFIRM()
{
  ...
  else if (layer == 2)
  {
    ...
    else if (layer2_Mode == SetMode) // 如果上次选择过了设置模式则关闭
    {
      ...
      else if (layer2PointerSetMenu == 4) // 如果光标在(温度显示)上
      {                                   // 如果光标在(温度显示)上
      ...
      else if (layer2PointerSetMenu == 3) // 如果光标在(设备通讯)上
      {
        if (isConnectToOtherDevice == true)
        {
          // 先反转
          isConnectToOtherDevice = false;
          // 写入内存卡进行保存
          WriteSDCardMessage("/root/SystemData", "MutilDeviceConn.mutil", (String)isConnectToOtherDevice);
          // 修改界面显示内容
          u8g2Fonts.setFont(u8g2_font_wqy16_t_gb2312);
          do
          {
            u8g2Fonts.setCursor(128, 142);
            u8g2Fonts.print("关闭");
          } while (display.nextPage());
        }
        else
      ...
      else if (layer2PointerSetMenu == 6) // 如果光标在(解锁增强)上
      {
        if (isUnlockPro == true)
        {
          // 先反转
          isUnlockPro = false;
          // 写入内存卡进行保存
          WriteSDCardMessage("/root/SystemData", "UnlockPro.unlock", (String)isUnlockPro);
          // 修改界面显示内容
          u8g2Fonts.setFont(u8g2_font_wqy16_t_gb2312);
          do
          {
            u8g2Fonts.setCursor(128, 238);
            u8g2Fonts.print("关闭");
          } while (display.nextPage());
        }
        else
    ...
    else if (layer2_Mode == ReadMode) // 如果上次选择过了阅读模式则关闭
    {
      ...
      else
      {
        ...
        // Serial.println("");
        TXTRead.Name = NowOpenTXTname; // 记录当前打开的TXT小说名称
        // 计算当前阅读书籍起始显示位置
        TXTRead.beginpos = (240 - (TXTRead.Name.length() - 4) * TXTRead.zitidaxiao) / 2;
        // 打开文件
  ...
  else if (layer == 3)
  {
    ...
    else if (layer3_Mode == ReadBookMode && layer3_Read_Mode_Set == true) // 如果在阅读书模式 设置菜单 单击用于确认当前操作
    {
      ...
      {
        WriteSDCardMessage("/root/SystemData", "ReadSetting.rs", (String)TXTRead.zijianju + "\r\n" + (String)TXTRead.hangjianju + "\r\n" + (String)TXTRead.yebianju + "\r\n" + (String)TXTRead.zitidaxiao + "\r\n" + (String)TXTRead.shiyongziti, false);
      ...
      else if (layer3PointerReadMode == 3) // 选择了 字间间距
      {
        if (TXTRead.zijianju < 16)
        {
          TXTRead.zijianju++;
        }
        else
        ...
        do
        {
          u8g2Fonts.setCursor(158, 20 + (25 * layer3PointerReadMode));
          u8g2Fonts.print(TXTRead.zijianju);
        } while (display.nextPage());
        delay(10);
        display.hibernate();
        // WriteSDCardMessage("/root/SystemData", "ReadSetting.rs", (String)TXTRead.zijianju + "\r\n" + (String)TXTRead.hangjianju + "\r\n" + (String)TXTRead.yebianju + "\r\n" + (String)TXTRead.zitidaxiao + "\r\n" + (String)TXTRead.shiyongziti, false);
      ...
      else if (layer3PointerReadMode == 4) // 选择了 行间间距
      {
        if (TXTRead.hangjianju < 16)
        {
          TXTRead.hangjianju++;
        }
        else
        {
          TXTRead.hangjianju = 1;
        }
        // 显示到后面
        u8g2Fonts.setFont(u8g2_font_wqy16_t_gb2312);
        do
        {
          u8g2Fonts.setCursor(158, 20 + (25 * layer3PointerReadMode));
          u8g2Fonts.print(TXTRead.hangjianju);
        } while (display.nextPage());
        delay(10);
        display.hibernate();
        // WriteSDCardMessage("/root/SystemData", "ReadSetting.rs", (String)TXTRead.zijianju + "\r\n" + (String)TXTRead.hangjianju + "\r\n" + (String)TXTRead.yebianju + "\r\n" + (String)TXTRead.zitidaxiao + "\r\n" + (String)TXTRead.shiyongziti, false);
      ...
      else if (layer3PointerReadMode == 5) // 选择了 页面边距
      {
        if (TXTRead.yebianju < 60)
        {
          TXTRead.yebianju++;
        }
        else
        ...
        {
          u8g2Fonts.setCursor(158, 20 + (25 * layer3PointerReadMode));
          u8g2Fonts.print(TXTRead.yebianju);
        } while (display.nextPage());
        delay(10);
        display.hibernate();
        // WriteSDCardMessage("/root/SystemData", "ReadSetting.rs", (String)TXTRead.zijianju + "\r\n" + (String)TXTRead.hangjianju + "\r\n" + (String)TXTRead.yebianju + "\r\n" + (String)TXTRead.zitidaxiao + "\r\n" + (String)TXTRead.shiyongziti, false);
      ...
      else if (layer3PointerReadMode == 6) // 选择了 字体大小
      {
        if (TXTRead.zitidaxiao < 16)
        {
          TXTRead.zitidaxiao++;
        }
        else
        {
          TXTRead.zitidaxiao = 12;
        }
        // 显示到后面
        u8g2Fonts.setFont(u8g2_font_wqy16_t_gb2312);
        do
        {
          u8g2Fonts.setCursor(158, 20 + (25 * layer3PointerReadMode));
          u8g2Fonts.print(TXTRead.zitidaxiao);
        } while (display.nextPage());
        delay(10);
        display.hibernate();
        // WriteSDCardMessage("/root/SystemData", "ReadSetting.rs", (String)TXTRead.zijianju + "\r\n" + (String)TXTRead.hangjianju + "\r\n" + (String)TXTRead.yebianju + "\r\n" + (String)TXTRead.zitidaxiao + "\r\n" + (String)TXTRead.shiyongziti, false);
      ...
      else if (layer3PointerReadMode == 7) // 选择了 字体选择
      {
        // WriteSDCardMessage("/root/SystemData", "ReadSetting.rs", (String)TXTRead.zijianju + "\r\n" + (String)TXTRead.hangjianju + "\r\n" + (String)TXTRead.yebianju + "\r\n" + (String)TXTRead.zitidaxiao + "\r\n" + (String)TXTRead.shiyongziti, false);
        // 暂时无用
      }

      else if (layer3PointerReadMode == 8) // 选择了向左移动一位
      {
        PagePercentageSelect = '<';
        do
        {
          display.drawRect(72, 217, 6, 3, GxEPD_BLACK);
          display.drawRect(87, 217, 6, 3, GxEPD_WHITE);
        } while (display.nextPage());
        delay(10);
        display.hibernate();
      }
      else if (layer3PointerReadMode == 9) // 选择了百分比减少
      {
        ...
        case '<':
          if (CharPagePercentage[0] > 1) // 十位减少，最小到0
          {
            ...
            do
            {
              u8g2Fonts.setCursor(72, 214);
              u8g2Fonts.print(CharPagePercentage[0]);
              u8g2Fonts.setCursor(87, 214);
      ...
      else if (layer3PointerReadMode == 10) // 选择了百分比增加
      {
        switch (PagePercentageSelect)
        {
        case '<':
      ...
      else if (layer3PointerReadMode == 11) // 选择了向右移一位
      {
        PagePercentageSelect = '>';
        do
        {
          display.drawRect(72, 217, 6, 3, GxEPD_WHITE);
          display.drawRect(87, 217, 6, 3, GxEPD_BLACK);
          display.drawRect(138, 217, 6, 3, GxEPD_WHITE);
        } while (display.nextPage());
        delay(10);
        display.hibernate();
      }
      else if (layer3PointerReadMode == 12) // 选择了确认跳页
      {
        layer3PointerReadMode = 0;
        // 计算跳页页数
        String i = (String)CharPagePercentage[0] + (String)CharPagePercentage[1];
        uint8_t j = atoi(i.c_str()); // 现在是百分比的状态
        //  返回阅读页面
        layer3_Read_Mode_Set = false;
        // 计算确认后的阅读位置
        uint32_t k = TXTRead.Get_p_file_int_all * j * 0.01; // 总字节数*跳页不带百分*0.01
        // 写入文件中
        WriteSDCardMessage("/root/BookDecode", TXTRead.new_i, (String)k, false);
        // 显示当前页
        MyTXTDecode(true);
      }
    }
...
void CLICK_DOWN()
{
  ...
  else if (layer == 2) // 如果在第二层
  {
    if (layer2_Mode == SetMode) // 如果在第二层的SetMode下
    {
      if (layer2PointerSetMenu != 6) // 如果在第二层SetMode下的光标所处位置不在最后一项下
      {
        layer2PointerSetMenu++;
        // 调用第二层光标显示函数
        SelectPionterSetMenu(); // 显示改变后的光标位置
...
void DOUBLE_CLICK_CONFIRM()
{
  ...
  else if (layer == 1) // 主界面 并且 阅读书籍 选项下双击，直接进入上一次阅读
  {
...
void setup()
{
  ...
  // delay(3000);
  // GET_ESPNOW_MSG(TimeMenuShow.temp);
  //     ESPNOWSTART();  //发送信息前的准备
  //     ESPNOW_SEND();  //发送信息
  ...
  // esp_sleep_enable_gpio_wakeup(GPIO_NUM_39, HIGH)
  //  新版本的唤醒方式修改为时钟芯片产生外部中断从而进行唤醒
  esp_sleep_enable_ext0_wakeup((gpio_num_t)RX8025T_INTV2, LOW); // 中断引脚低电平时候产生唤醒
                                                                // 旧版本的唤醒方式为内置定时器进行唤醒，存在各种问题
  // 1 = High, 0 = Low //设置睡眠唤醒引脚
  esp_sleep_enable_ext1_wakeup(0x8000000000, ESP_EXT1_WAKEUP_ANY_HIGH);

  // 判断唤醒源
  if (esp_sleep_get_wakeup_cause() == ESP_SLEEP_WAKEUP_EXT0) // 被定时器唤醒原因：时钟定时器 旧：ESP_SLEEP_WAKEUP_TIMER 新：
  {
    // 判断刷新次数，与ESPNOW设备间进行通讯
    if (isConnectToOtherDevice == true && PartialRefresh_Times > 5) // 每5分钟进行一次通讯
    {
      GET_ESPNOW_MSG(T.c_str());
      ESPNOWSTART(); // 发送信息前的准备
      ESPNOW_SEND(); // 发送信息
    }

    PartialRefresh_Times++;
    display.epd2.selectSPI(hspi, SPISettings(15000000, MSBFIRST, SPI_MODE0));
  ...
  else
  {
    ...
    {
      // 打开定时器
      uint32_t timerUnlockPro = millis();
      // 判断打开了解锁增强
      // 等待1S内判断是否按下了规定按键
      while (1)
      {
        uint32_t timerUnlockProIn = millis();
        if (digitalRead(BTNIO_DOWN) == HIGH && timerUnlockProIn - timerUnlockPro <= 60 * 1000) // 如果指定按键在规定时间内被按下
        {
          break;
        }
        else
    ...
    // 时钟中断
    pinMode(RX8025T_INTV2, INPUT);
    RTC_RX8025T.initTUI(INT_MINUTE); // 打开分钟中断
    ...
    else
    {
      ...
      else // 否则存在，则可以判断是否存在其余文件
      {
        ...
        // 读取阅读设置部分
        if (SD.exists("/root/SystemData/ReadSetting.rs"))
        {
          File file = SD.open("/root/SystemData/ReadSetting.rs", "r");
          String set = file.readString();
          // Serial.println(set);
          file.close();
          int place0 = set.indexOf("\r\n");
          int place1 = set.indexOf("\r\n", place0 + 2);
          int place2 = set.indexOf("\r\n", place1 + 2);
          int place3 = set.indexOf("\r\n", place2 + 2);
          int place4 = set.indexOf("\r\n", place3 + 2);
          String zjj = set.substring(0, place0);
          String hjj = set.substring(place0 + 2, place1);
          String ybj = set.substring(place1 + 2, place2);
          String ztdx = set.substring(place2 + 2, place3);
          String ztxz = set.substring(place3 + 2, set.length());
          TXTRead.zijianju = zjj.toInt();
          TXTRead.hangjianju = hjj.toInt();
          TXTRead.yebianju = ybj.toInt();
          TXTRead.zitidaxiao = ztdx.toInt();
          TXTRead.shiyongziti = ztxz.toInt(); // 字体选择（暂时无用）
        }
        // 读取系统配置文件 (是否设备联动)
        if (SD.exists("/root/SystemData/MutilDeviceConn.mutil"))
        {
          File file = SD.open("/root/SystemData/MutilDeviceConn.mutil", "r");
          String content = file.readString();
          file.close();
          isConnectToOtherDevice = (content == "1") ? true : false;
        }
        // 读取系统配置文件 (是否解锁增强)
        if (SD.exists("/root/SystemData/UnlockPro.unlock"))
        {
          File file = SD.open("/root/SystemData/UnlockPro.unlock", "r");
          String content = file.readString();
          file.close();
          isUnlockPro = (content == "1") ? true : false;
        }
      }
    }
...
void loop()
{
  ...
  if ((layer == 3 && layer3_Mode == ReadBookMode) || layer == 1)
  {
    TXT_nowTime = millis();
...
void SelectPionterSetMenu()
{
  ...
  if (layer == 2 && layer2_Mode == SetMode)
  {
    ...
    else if (layer2PointerSetMenu == 1) // 光标显示在第二个选项上(联网设置)
    {
      // 显示光标在第二个按键上
      do
      {
        ...
        display.drawRoundRect(5, 95, 105, 19, 3, GxEPD_WHITE);
      } while (display.nextPage());
      display.hibernate();
    }

    else if (layer2PointerSetMenu == 2) // 光标显示在第三个选项上()
    {
      // 显示光标在第三个按键上
      do
      {
        display.drawRoundRect(5, 63, 105, 19, 3, GxEPD_WHITE);
        display.drawRoundRect(5, 95, 105, 19, 3, GxEPD_BLACK);
        display.drawRoundRect(5, 127, 105, 19, 3, GxEPD_WHITE);
      } while (display.nextPage());
      display.hibernate();
    }
    ...
    else if (layer2PointerSetMenu == 3) // 光标显示在第二个选项上()
    {
      ...
      {
        display.drawRoundRect(5, 95, 105, 19, 3, GxEPD_WHITE);
        display.drawRoundRect(5, 127, 105, 19, 3, GxEPD_BLACK);
        display.drawRoundRect(5, 159, 105, 19, 3, GxEPD_WHITE);
      } while (display.nextPage());
      display.hibernate();
    }
    ...
    else if (layer2PointerSetMenu == 4) // 光标显示在第二个选项上()
    {
      ...
      {
        display.drawRoundRect(5, 127, 105, 19, 3, GxEPD_WHITE);
        display.drawRoundRect(5, 159, 105, 19, 3, GxEPD_BLACK);
        display.drawRoundRect(5, 191, 105, 19, 3, GxEPD_WHITE);
      } while (display.nextPage());
      display.hibernate();
    }
    ...
    else if (layer2PointerSetMenu == 5) // 光标显示在第二个选项上()
    {
      ...
      {
        display.drawRoundRect(5, 159, 105, 19, 3, GxEPD_WHITE);
        display.drawRoundRect(5, 191, 105, 19, 3, GxEPD_BLACK);
        display.drawRoundRect(5, 223, 105, 19, 3, GxEPD_WHITE);
      } while (display.nextPage());
      display.hibernate();
    }

    else if (layer2PointerSetMenu == 6) // 光标显示在第六个选项上()
    {
      // 显示光标在第二个按键上
      do
      {
        display.drawRoundRect(5, 191, 105, 19, 3, GxEPD_WHITE);
        display.drawRoundRect(5, 223, 105, 19, 3, GxEPD_BLACK);
      } while (display.nextPage());
      display.hibernate();
    }
  }
...
int TimeShow()
{
  ...
  // 温湿度显示区域
  if(isOpenTempHumid == true) {
    if(sht30.get() == 0) {  // 获取传感器数据
      T = String(sht30.cTemp, 1) + "°C";
      Humidity = String(sht30.humidity, 1) + "%";  // 获取湿度数据
    }
  }
  ...
  if (isOpenTempHumid == true)
  {
    ...
    u8g2Fonts.print(T);
    u8g2Fonts.setCursor(180, 50); 
    u8g2Fonts.print(Humidity);  // 显示湿度
  }
  
  pinMode(WAKEIO, OUTPUT);
  digitalWrite(WAKEIO, HIGH);
  // 该处调用电量计算显示函数
  int8_t powerString = BATCaculate();
  if (powerString > 100)
  {
    powerString = 100;
  }
  ...
  Datesum = "20" + String(Year) + "." + String(Month) + "." + String(Date) + "    -" + String(DoW) + "-";
  ...
  do
  {
    ...
    u8g2Fonts.setCursor(130, 223);
    u8g2Fonts.print(Datesum);

    u8g2Fonts.setCursor(240, 223);
    u8g2Fonts.print(powerString);
    u8g2Fonts.print("%");
    ...
    if (isOpenTempHumid == true)
    {
      ...
      if (sht30.get() == 0)
      {
        ...
        TimeMenuShow.temp = T;
        T = T + " C";
        Humidity = (String)(sht30.humidity - HumidOffest);
        Humidity = Humidity + " %";
...
void SetModeShow()
{
  ...
  do
  {
    ...
    for (int i = 0; i < 6; i++)
    {
      u8g2Fonts.setCursor(8, 78 + i * 32); // 38->32
      u8g2Fonts.print(SMDM.Setmenu[i]);
    }

    // 下面区域用于显示软件版本号和其他东西
    display.fillRect(25, 250, 190, 2, GxEPD_BLACK);
    u8g2Fonts.setCursor(38, 278);
    u8g2Fonts.printf("发布日期：\t %d - %d - %d", VersionEPD.Yearsion, VersionEPD.Version, VersionEPD.Release);
    u8g2Fonts.setCursor(38, 308);
    u8g2Fonts.print("制作人员：\t pxy");
    u8g2Fonts.setCursor(38, 338);
    u8g2Fonts.print("bilibili   ：\t pp");
    u8g2Fonts.setCursor(38, 368);
    u8g2Fonts.printf("MAC：\t %s", MACAddr.c_str());
    u8g2Fonts.setCursor(38, 398);
    u8g2Fonts.print("ChipID： \t");
    u8g2Fonts.print(ChipIDxxxxx);
...
void ReadModeReadShowMenu()
{
  PagePercentage = TXTRead.Get_p_file_int * 100 / (TXTRead.Get_p_file_int_all); // 当前指针位置/（总体指针/3）用于估算还有多少没读
  ...
  do
  {
    ...
    for (int i = 0; i < 8; i++)
    {
      ...
      if (i < 3)
      {
        u8g2Fonts.setCursor(94, beginypos + (25 * i));
        u8g2Fonts.print(RMDM.Readmenu[i].c_str());
      }
      else
      {
        u8g2Fonts.setCursor(50, beginypos + (25 * i));
        u8g2Fonts.print(RMDM.Readmenu[i].c_str());
      }
    }
    ...
    {
      // 显示选项对应的默认参数
      u8g2Fonts.setCursor(160, beginypos + (25 * i));
      switch (i)
      {
      case 3:
        u8g2Fonts.print(TXTRead.zijianju);
    ...
    u8g2Fonts.setCursor(40, 214);
    u8g2Fonts.print("<");
    u8g2Fonts.setCursor(55, 214);
    u8g2Fonts.print("-");
    ...
    u8g2Fonts.setCursor(102, 214);
    u8g2Fonts.print("%");
    u8g2Fonts.setCursor(115, 214);
    u8g2Fonts.print("+");
    u8g2Fonts.setCursor(129, 214);
    u8g2Fonts.print(">");

    u8g2Fonts.setCursor(142, 214);
    u8g2Fonts.print("E"); // 显示Enter

    display.drawRoundRect(64, 4, 117, 19, 3, GxEPD_BLACK); // 自动框选十位
    // 显示电量
...
void ReadModeReadShowMenu_Rectangle()
{
  ...
  if (layer == 3 && layer3_Mode == ReadBookMode && layer3_Read_Mode_Set == true)
  {
    display.setPartialWindow(0, 0, display.width(), display.height());
    // 判断选择的哪个选项
    if (layer3PointerReadMode == 0) // 在继续阅读选项上
    {
      do
      {
        display.drawRoundRect(64, 29, 117, 19, 3, GxEPD_WHITE); // 清除圆角
        display.drawRoundRect(64, 4, 117, 19, 3, GxEPD_BLACK);  // 绘制圆角
      } while (display.nextPage());
      display.hibernate();
    }
    else if (layer3PointerReadMode == 1) // 在上级界面上
    {
      do
      {
        display.drawRoundRect(64, 54, 117, 19, 3, GxEPD_WHITE); // 清楚圆角
        display.drawRoundRect(64, 4, 117, 19, 3, GxEPD_WHITE);  // 清楚圆角
        display.drawRoundRect(64, 29, 117, 19, 3, GxEPD_BLACK); // 绘制圆角
      } while (display.nextPage());
      display.hibernate();
    }
    else if (layer3PointerReadMode == 2) // 在回主界面上
    {
      do
      {
        display.drawRoundRect(64, 29, 117, 19, 3, GxEPD_WHITE); // 清楚圆角
        display.drawRoundRect(64, 54, 117, 19, 3, GxEPD_BLACK); // 绘制圆角
        display.drawRoundRect(20, 79, 117, 19, 3, GxEPD_WHITE); // 清楚圆角

      } while (display.nextPage());
      display.hibernate();
    }
    else if (layer3PointerReadMode == 3) // 在字间间距上
    {
      do
      {
        display.drawRoundRect(64, 54, 117, 19, 3, GxEPD_WHITE);  // 清楚圆角
        display.drawRoundRect(20, 79, 117, 19, 3, GxEPD_BLACK);  // 绘制圆角
        display.drawRoundRect(20, 104, 117, 19, 3, GxEPD_WHITE); // 清楚圆角
      } while (display.nextPage());
      display.hibernate();
    }
    else if (layer3PointerReadMode == 4) // 在行间间距上
    {
      do
      {
        display.drawRoundRect(20, 79, 117, 19, 3, GxEPD_WHITE);  // 清楚圆角
        display.drawRoundRect(20, 104, 117, 19, 3, GxEPD_BLACK); // 绘制圆角
        display.drawRoundRect(20, 129, 117, 19, 3, GxEPD_WHITE); // 清楚圆角
      } while (display.nextPage());
      display.hibernate();
    }
    else if (layer3PointerReadMode == 5) // 在页面边距上
    {
      do
      {
        display.drawRoundRect(20, 104, 117, 19, 3, GxEPD_WHITE); // 清楚圆角
        display.drawRoundRect(20, 129, 117, 19, 3, GxEPD_BLACK); // 绘制圆角
        display.drawRoundRect(20, 154, 117, 19, 3, GxEPD_WHITE); // 清楚圆角
      } while (display.nextPage());
      display.hibernate();
    }
    else if (layer3PointerReadMode == 6) // 在字体大小上
    {
      do
      {
        display.drawRoundRect(20, 129, 117, 19, 3, GxEPD_WHITE); // 清楚圆角
        display.drawRoundRect(20, 154, 117, 19, 3, GxEPD_BLACK); // 绘制圆角
        display.drawRoundRect(20, 179, 117, 19, 3, GxEPD_WHITE); // 清楚圆角

      } while (display.nextPage());
      display.hibernate();
    }
    else if (layer3PointerReadMode == 7) // 在使用字体上
    {
      do
      {
        display.drawRoundRect(20, 154, 117, 19, 3, GxEPD_WHITE); // 清楚圆角
        display.drawRoundRect(20, 179, 117, 19, 3, GxEPD_BLACK); // 绘制圆角
        display.drawLine(40, 218, 47, 218, GxEPD_WHITE);         // 清楚下一个位置
      } while (display.nextPage());
      display.hibernate();
    }
    else if (layer3PointerReadMode == 8) // 在左移一位
    {
      do
      {
        display.drawRoundRect(20, 179, 117, 19, 3, GxEPD_WHITE); // 清楚圆角
        display.drawLine(40, 218, 47, 218, GxEPD_BLACK);
        display.drawLine(54, 218, 61, 218, GxEPD_WHITE);
      } while (display.nextPage());
      display.hibernate();
    }
    else if (layer3PointerReadMode == 9) // 在页数减少
    {
      do
      {
        display.drawLine(40, 218, 47, 218, GxEPD_WHITE);
        display.drawLine(54, 218, 61, 218, GxEPD_BLACK);
        display.drawLine(115, 218, 121, 218, GxEPD_WHITE);
      } while (display.nextPage());
      display.hibernate();
    }
    else if (layer3PointerReadMode == 10) // 在页数增加
    {
      do
      {
        display.drawLine(54, 218, 61, 218, GxEPD_WHITE);
        display.drawLine(115, 218, 121, 218, GxEPD_BLACK);
        display.drawLine(127, 218, 134, 218, GxEPD_WHITE);
      } while (display.nextPage());
      display.hibernate();
    }
    else if (layer3PointerReadMode == 11) // 在右移一位
    {
      do
      {
        display.drawLine(115, 218, 121, 218, GxEPD_WHITE);
        display.drawLine(127, 218, 134, 218, GxEPD_BLACK);
        display.drawLine(142, 218, 148, 218, GxEPD_WHITE);
      } while (display.nextPage());
      display.hibernate();
    }
    else if (layer3PointerReadMode == 12) // 在确认跳页按键上
    {
      do
      {
        display.drawLine(127, 218, 134, 218, GxEPD_WHITE);
        display.drawLine(142, 218, 148, 218, GxEPD_BLACK);
      } while (display.nextPage());
      display.hibernate();
    }
    // display.hibernate();
...
String GET_OPEN_TXT_NAME()
{
  ...
  TXTRead.Get_p_file_int_all = txtFile.size();
  txtFile.close();
  return getTXT;
}
...
{
  // 判断文件是否被打开过
  TXTRead.new_i = bookname + ".newi";
  TXTRead.old_i = bookname + ".i";
  if (SD.exists("/root/BookDecode/" + TXTRead.new_i))
  {
    // 先判断是否存在最新版本的解析文件(用于已存在旧版本解析文件的机器过渡到新版本解析上来)
  ...
  else if (SD.exists("/root/BookDecode/" + TXTRead.old_i))
  {
    Serial.println("存在旧版解析文件");
    // 判断机器是否存在旧版本的解析文件
    // 如果存在旧版本的解析文件,则提取旧版本解析文件的指针位置
    String getpage;
    uint32_t getpageuint;
    indexesFile = SD.open("/root/BookDecode/" + TXTRead.old_i, "r"); // 打开索引文件
    ...
    uint32_t txt_syys_uint32 = atol(getpage.c_str()); // 转换成int格式
    getpageuint = (txt_syys_uint32 - 2) * 7 - 7;
    indexesFile.close();
    // 成功提取到当前页首位置指针之后写入文件内
    WriteSDCardMessage("/root/BookDecode", TXTRead.new_i, (String)getpageuint, false);
    // 打开小说
    layer = 3;                  // 第三层
    layer3_Mode = ReadBookMode; // 设置第二层模式为ReadBookMode
    MyTXTDecode(true);
  ...
  {
    Serial.println("完全的新书");
    // 否则完全就是本新书，一个解析文件都不存在
    // 创建新版本的解析文件,将指针置为1
    WriteSDCardMessage("/root/BookDecode", TXTRead.new_i, "1", false);
    // 打开小说
    layer = 3;                  // 第三层
    layer3_Mode = ReadBookMode; // 设置第二层模式为ReadBookMode
    MyTXTDecode(true);
...
// 用于向SD卡某文件位置，写入某些信息 FilePlace:文件位置 FileName:文件名称 FileContent：文件内容
void WriteSDCardMessage(String FilePlace, String Filename, String FileContent, bool lazha)
{
#if versionV371 == 2
  if (!SD.begin(SD_CS, sdspi))
  {
...
void MyTXTDecode(bool place)
{
  ...
  // 获取上一次阅读的位置
  File Get_p_file = SD.open("/root/BookDecode/" + TXTRead.new_i, "r");
  String Get_p_file_String = Get_p_file.readString();       // 从文件读取出字符串形式
  TXTRead.Get_p_file_int = atol(Get_p_file_String.c_str()); // 转化为int32形式
  Get_p_file.close();
  // 打开小说文件
  txtFile = SD.open("/root/OthersData/" + TXTRead.Name, "r");
  // 偏移阅读位置指针到上次阅读位置上
  txtFile.seek(TXTRead.Get_p_file_int);
  // 重置一次阅读计时器
  TXT_overTime = millis();

  // 本文件需要根据 字间距，行间距，字体大小， 来进行独立计算显示的内容，并交给TXT数组

  // 怎样计算一行能够容纳多少字呢？
  // 从前往后翻页和后往前翻页计算方法不太相同

  // 先计算一行剩余多少橡塑用于容纳 带间距的字体
  int RemainLengthPIX = TXTRead.ScreenLength - 2 * TXTRead.yebianju; // （240像素 - 2*页边距 = 剩余的像素）
  // 再计算一页剩余多少像素用于容纳
  int RemainHeigthPIX = TXTRead.ScreenHeight - 2 * TXTRead.yebianju - 30; // （416像素 - 2*页边距 -30固定底边栏 = 剩余的像素）
  // 再通过剩余的计算至少能够容纳多少字体 计算的是全为汉字的时候的情况，如果有符号出现会更短
  int HangRongNaZiShu = RemainLengthPIX / (TXTRead.zitidaxiao + TXTRead.zijianju); // 未考虑为奇数和偶数的情况
  // 再通过计算算出当前一页能够存放多少行
  int RongNaHangShu = RemainHeigthPIX / (TXTRead.zitidaxiao + TXTRead.hangjianju);        // 剩余列像素/（字体大小 + 行间距）
  int YUSHURongNaHangShu = (RemainHeigthPIX % (TXTRead.zitidaxiao + TXTRead.hangjianju)); // 剩余列像素的余数，用于左右均衡页边距
  // 计算两侧实际开始位置
  int LiangCeShiJi_FuJPY = (YUSHURongNaHangShu * (TXTRead.zitidaxiao + TXTRead.hangjianju) * 0.1) / 2;
  Serial.print("两侧偏移:");
  Serial.println(LiangCeShiJi_FuJPY);
  // 经过上面计算后就可以知道需要多大的字符串了
  String TXTzifuchuan[RongNaHangShu + 1];

  // 往前还是后翻页的处理
  bool upORnext = place;

  if (upORnext == true) // 如果是向后翻页
  {
    if (QIAN_fanyecishu != 0)
    {
      QIAN_fanyecishu = 0; // 取消标记正在向后翻页
      // 写入开始位置指针
      txtFile.seek(HOU_fanyeqianweizhi, SeekSet);
    }
    else
    {
      HOU_fanyeqianweizhi = txtFile.position(); // 获取下一个即将被读取字节的位置
    }
    ...
    while (1) // 向后翻页
    {
      for (int i = 0; i < RongNaHangShu; i++)
      {
        for (int j = 0; j < HangRongNaZiShu; j++)
        {
          // 读取一个字节
          char c = txtFile.read();
          // 先检查是否为换行符
          ...
          else
          {
            ...
            else if (c == '\t') // 如果是水平制表符，则代替两个中文位置
            {
              // Serial.println("制表符，本行结束");
              TXTzifuchuan[i] += "  ";
            }
            else if (c >= 31 && c <= 199)
            {
          ...
          // 读够至少能够容纳的字符数，不知道标点符号是否会占用字体宽度这么多位置
          // 假设标点符号也占用这么多位置
        }
      }
  ...
  {
    // 判断前翻页次数
    if (QIAN_fanyecishu >= 1)
    {
      // 交换页面末端字符位置
      HOU_fanyeqianweizhi = QIAN_fanyeqianweizhi;
      // 先定义到上一页的位置
      txtFile.seek(QIAN_fanyeqianweizhi, SeekSet);
    }
    else
    {
      if (txtFile.position() == 1) // 首页直接跳出
      {
        return;
      }
    ...
    // Serial.print("开始位置:");
    // Serial.println(HOU_fanyeqianweizhi);

    while (1) // 向前翻页,通过计算符号数量来找到上一页最初的位置
    {
      const int BUFFER_SIZE = 1024;                           // TXT文件缓冲区大小
      char buffer[BUFFER_SIZE];                               // 缓冲区
      txtFile.seek(-1024, SeekCur);                           // 定位到1024之前
      int bytesRead = txtFile.readBytes(buffer, BUFFER_SIZE); // 读取内容到缓冲区
      Serial.print("开始位置:");
      Serial.println(txtFile.position());
      if (bytesRead > 0)
      {
        ...
        for (int i = 0; i < RongNaHangShu; i++)
        {
          int dangqianhangzishu = 0;
          // int32_t beginpos = txtFile.position();
          for (int j = 0; j < HangRongNaZiShu; j++)
          {
            if (dangqianhangzishu >= HangRongNaZiShu) // 如果当前行字数和本行能够容纳的字数量相同
            {
              break;
            }

            // 对缓冲区中的每个字符进行操作
            char c = buffer[1024 - bufferi];
            ...
            if ((c & 0x80) == 0)
            {
              ...
              else if (c == '\n')
              {
                ...
                // 处理换行符的逻辑
                // 向前移动一个指针位置
                // txtFile.seek(beginpos - 1);
                bufferi++; // 向前移动一个字符
                // txtFile.seek(-2, SeekCur);
                break; // 此行结束
              }
              else if (c == ' ')
              {
                ...
                // 处理空格的逻辑
                bufferi++;           // 向前移动一个字符
                dangqianhangzishu++; // 本行字数增加1
              }
              else if (c >= 32 && c <= 199) // 如果是能够被显示出来的符号
              {
                // 处理单字节字符的逻辑
                bufferi++;           // 向前移动一个字符
                dangqianhangzishu++; // 本行字数加1
              }
              else
              {
                bufferi++; // 向前移动一个字符
            ...
            else if ((c & B10000000) == B10000000) // 三字符
            {
              // 判断前一个是否依然为3字符
              bufferi++;
              char a = buffer[1024 - bufferi];
              if ((a & B10000000) == B10000000) // 三字符的第二位判断成功则说明这是一个标准的汉字
              {
                bufferi += 2; // 再向前移动两个字符
              }
              else if ((a & B11100000) == B11100000) // 三字符的第二位直接就为首位汉字编码，则说明读漏了一位，指针只向前移动一位
              {
                bufferi++;
              }
              dangqianhangzishu++;
        ...
        // 此时得出来的bufferi就是上一页面的开头
        txtFile.seek(-bufferi + 1, SeekCur);
        QIAN_fanyeqianweizhi = txtFile.position(); // 获取下一个即将被读取字节的位置
        Serial.print("结束位置:");
        Serial.println(QIAN_fanyeqianweizhi);
      }
      else
      {
        // 文件读取失败或已到达文件末尾
  ...
  // 发送给显示函数
  switch (TXTRead.zitidaxiao)
  {
  case 12:
    u8g2Fonts.setFont(u8g2_font_wqy12_t_gb2312);
    break;
  case 13:
    u8g2Fonts.setFont(u8g2_font_wqy13_t_gb2312);
    break;
  case 14:
    u8g2Fonts.setFont(u8g2_font_wqy14_t_gb2312);
    break;
  ...
  if (1)
  {
    do
    {
      for (uint8_t i = 0; i < RongNaHangShu; i++)
      {
        // 为每个符号确定位置
        int xcorsur = 0;
        for (int j = 0; j < TXTzifuchuan[i].length(); j++)
        {
          // xcorsur ++;
          u8g2Fonts.setCursor(TXTRead.yebianju + LiangCeShiJi_FuJPY / 2 + xcorsur * (TXTRead.zijianju + TXTRead.zitidaxiao), (i + 1) * (TXTRead.hangjianju + TXTRead.zitidaxiao) + TXTRead.yebianju);

          char c = TXTzifuchuan[i][j];
          boolean asciiState = 0;
          byte a = B11100000;
          byte b = c & a;
          ...
          if (b == B11100000) // 如果是中文 则3个字节
          {
            char cc = TXTzifuchuan[i][j];
            String pp;
            pp += cc;
            cc = TXTzifuchuan[i][j + 1];
            pp += cc;
            cc = TXTzifuchuan[i][j + 2];
            pp += cc;
          ...
          {
            u8g2Fonts.print(TXTzifuchuan[i][j]);
          }
          xcorsur++;
        }

        // u8g2Fonts.setCursor(yebianju, (i + 1) * (hangjianju + zitidaxiao) + yebianju);
        // u8g2Fonts.print(TXTzifuchuan[i]);
        // Serial.println(beginpos);
        u8g2Fonts.setCursor(TXTRead.beginpos, 410);
        u8g2Fonts.print("《");
        u8g2Fonts.print(TXTRead.Name);
        u8g2Fonts.print("》");
      }
    } while (display.nextPage());
  }
  ...
  WriteSDCardMessage("/root/BookDecode", TXTRead.new_i, (String)txtFile.position(), false);
  // FixedRefresh();
}
...
void displayTXT()
{
  ...
  do
  {
    display.fillRect(0, 300, 240, 116, GxEPD_WHITE);
  } while (display.nextPage());
  display.hibernate();
  ...
  if (1)
  {
    do
    {
      for (uint8_t i = 0; i < 3; i++) // 仅仅显示三行
      {
        // 为每个符号确定位置
        int xcorsur = 0;
        for (int j = 0; j < HangRongNaZiShu; j++) // 以行容纳字数为界限
        {
          u8g2Fonts.setCursor(TXTRead.yebianju + LiangCeShiJi_FuJPY / 2 + xcorsur * (TXTRead.zijianju + TXTRead.zitidaxiao), (i + 1) * (TXTRead.hangjianju + TXTRead.zitidaxiao) + TXTRead.yebianju + 300);
          u8g2Fonts.print("我");
          xcorsur++;
        }
      }
